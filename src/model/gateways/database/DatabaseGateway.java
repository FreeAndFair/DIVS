package model.gateways.database;
...
import model.data.BallotJournal;
import model.data.Candidate;
...
public /*@ nullable_by_default @*/ class DatabaseGateway implements DB_interface {
	...
	public /*@ pure @*/ Ballot getBallot(/*@ non_null @*/ PollingStation ps) throws DivsException {
		...
		try {
			...
			select = "select p.party_id, p.name, p.standing, case when p.letter is not null then p.letter else (select p2.letter from Party p2 where p2.party_id = c.party_id) end as letter, c.candidate_id, c.name, c.cpr, c.position, c.address, bj.bj_id, c.party_id " +
			...
			while (res.next()) {
				...
				if (party_id > 0) {
					p = new Party(party_id, res.getString(2), 
							res.getString(4), res.getInt(3));
					BallotJournal bjp = new BallotJournal(p, res.getInt(10));
					journal.add(bjp);
				} else {
					if (res.getInt(11) == 0) {p = null;} // independent candidates
	...
	public void registerResult(/*@ non_null @*/ Ballot ballot,
		...
		try {
			...
			for (int i = 0; i < ballot.getJournal().size(); i++) {
				...
				pstmt.setInt(4, bj.getPartyVotes());
				pstmt.setInt(5, bj.getPersonalVotes());
				pstmt.executeUpdate();
			}
	 ...
	 */
	@Override
	public /*@ pure @*/ List<ConstituencyResult> getConstituencyResults(int temp_final) throws DivsException {
	 ...
	 */
	@Override
	public /*@ pure @*/ List<PartyResult> getNationalResults(int temp_final) throws DivsException {
		List<Party> parties = getParties();
		try {
	 ...
	 */
	@Override
	public /*@ pure @*/ List<ProvinceResult> getProvinceResults(int temp_final) throws DivsException {
		List<Province> provinces = this.getProvinces();
		List<Party> parties = getParties();
		try {
	 ...
	 */
	@Override
	public void	addCandidatesToConstituencyResults(List<ConstituencyResult> const_result) throws DivsException {
		List<Party> parties = getParties();
