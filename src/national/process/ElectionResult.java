...
import national.data.computation.Div;
...
public /*@ nullable_by_default @*/ class ElectionResult {
	...
	//@ invariant additional_seats_taken >= 0 && additional_seats_taken <= ElectionConstants.ADDITIONAL_SEATS;
	...
	/**
	 * @return the constituency_resultss
	 */
	public /*@ pure @*/ List<ConstituencyResult> getConstituencyResult() {
	 ...
	 */
	//@ post constituency_resultss == const_res;
	//@ assignable constituency_resultss;
	...
	//@ pre constituency_resultss.size() == ElectionConstants.NO_OF_CONSTITUENCIES; // (1)
	//@ pre \forall int j; 0 <= j && j < constituency_resultss.size(); constituency_resultss.get(j).getSeatsTaken() == 0;
	//@ pre \forall int j; 0 <= j && j < constituency_resultss.size(); \forall int k; 0 <= k && k < constituency_resultss.get(j).getPartyResult().size(); constituency_resultss.get(j).getPartyResult().get(k).getConstituencySeats() == 0 && constituency_resultss.get(j).getPartyResult().get(k).getAdditionalSeats() == 0 && constituency_resultss.get(j).getPartyResult().get(k).getAdditionalSeatsTaken() == 0;
	//@ pre \forall int j; 0 <= j && j < constituency_resultss.size(); \forall int k; 0 <= k && k < constituency_resultss.get(j).getIndependentCandidates().size(); !constituency_resultss.get(j).getIndependentCandidates().get(k).isElected();
	//@ pre \forall int i; 0 <= i && i < constituency_resultss.size(); \forall int j; 0 <= j && j < constituency_resultss.get(i).getPartyResult().size(); \exists int n; 0 <= n && n < national_results.size(); constituency_resultss.get(i).getPartyResult().get(j).getParty().getPartyId() == national_results.get(n).getParty().getPartyId();     
	//@ pre \forall int j; 0 <= j && j < constituency_resultss.size(); ((\exists int k; 0 <= k && k < constituency_resultss.get(j).getPartyResult().size(); constituency_resultss.get(j).getPartyResult().get(k).getTotalVotes() > 0) || (constituency_resultss.get(j).getElectableIndepCand >= constituency_resultss.get(j).getConstituency().getConstituencySeats()));
	//@ post inner_status == ElectionStatus.STEP_1_DONE || inner_status == ElectionStatus.STEP_1_RESOLVED;
	//@ post \forall int j; 0 <= j && j < constituency_resultss.size(); constituency_resultss.get(j).getSeatsTaken() >= constituency_resultss.get(j).getConstituency().getConstituencySeats();
	//@ post (\exists int i; 0 <= i && i < constituency_resultss.size(); \exists int j; 0 <= j && j < constituency_resultss.get(i).getPartyResult().size(); constituency_resultss.get(i).getPartyResult().get(j).isDraw() ==> ((\exists int k; 0 <= k && k < constituency_resultss.get(i).getPartyResult().size(); constituency_resultss.get(i).getPartyResult().get(k).isDraw() && j != k)) || (\exists int m; 0 <= m && m < constituency_resultss.get(i).getIndependentCandidates().size(); constituency_resultss.get(i).getIndependentCandidates().get(m).isDraw()));
	//@ post (\exists int i; 0 <= i && i < constituency_resultss.size(); \exists int j; 0 <= j && j < constituency_resultss.get(i).getIndependentCandidates().size(); constituency_resultss.get(i).getIndependentCandidates().get(j).isDraw() ==> ((\exists int k; 0 <= k && k < constituency_resultss.get(i).getPartyResult().size(); constituency_resultss.get(i).getPartyResult().get(k).isDraw()) || (\exists int m; 0 <= m && m < constituency_resultss.get(i).getIndependentCandidates().size(); constituency_resultss.get(i).getIndependentCandidates().get(m).isDraw() && j != m));
	//@ post !isStep1Resolved() ==> inner_status == ElectionStatus.STEP_1_DONE;
	//@ post !isStep1Resolved() ==> (\exists int i; 0 <= i && i < constituency_resultss.size(); (\exists int j; 0 <= j && j < constituency_resultss.get(i).getPartyResult().size(); constituency_resultss.get(i).getPartyResult().get(j).isDraw() && (\exists int k; 0 <= k && k < constituency_resultss.get(i).getPartyResult().size(); constituency_resultss.get(i).getPartyResult().get(j).isDraw() && j != k)));
	//@ post isStep1Resolved() ==> inner_status == ElectionStatus.STEP_1_RESOLVED;
	//@ assignable inner_status, constituency_resultss;
	public void allocate_constituency_seats() {
		/*
	...
	/**
	 * Step 1 (if too many seats are taken in some constituency/ies):
	 * This method prunes the list of elected down to the total number of 
	 * constituency seats in Denmark. This should be something which is done
	 * manually, but to speeed up tests etc. this is done here.
	 * N.B. This should be a (pseudo) random algorithm, but since this in 
	 * the first place should not be a part of the application, then first 
	 * party in the list is always favoured
	 */
	//@ pre inner_status == ElectionStatus.STEP_1_DONE;
	//@ pre \exists int j; 0 <= j && j < constituency_results.size(); constituency_results.get(j).getSeatsTaken() > constituency_results.get(j).getConstituency().getConstituencySeats();
	...
	//@ post inner_status == ElectionStatus.STEP_1_RESOLVED;
	//@ assignable inner_status, constituency_results;
	public void auto_resolve_constituency_seats() {
		
		ConstituencyResult cr;
		PartyResult pres;
		
		/*
		 * Here there are too many seats taken in some constituencies,
		 * and therefore some pruning action must be performed
		 */
		for (int i = 0; i < constituency_results.size(); i++) {
			cr = constituency_results.get(i);
			while(constituency_results.get(i).getSeatsTaken() > 
					cr.getConstituency().getConstituencySeats()) {

				/*
				 * List will be of size larger than 1 as long 
				 * as there are multiple parties still involved
				 */
				List<PartyResult> parties = this.getPartyDraw(cr);
				if (parties != null && parties.size() > 1) {
					pres = parties.get(parties.size()-1); 
					pres.setDraw(false);
					pres.setConstituencySeats(pres.getConstituencySeats() - 1);
					constituency_results.get(i).setSeatsTaken(constituency_results.get(i).getSeatsTaken() - 1);
					break;
				} else {
					break;
				}
	...
	//@ post \forall int i; 0 <= i && i < national_results.size(); \exists int j; 0 <= j && j < province_results.size(); \exists int k; 0 <= k && province_results.get(j).getPartyResult().size(); province_results.get(j).getPartyResult().get(k).getParty().getPartyId() == national_results.get(i).getParty().getPartyId() && province_results.get(j).getPartyResult().get(k).getTotalVotes() >= (int)((double)province_results.get(j).getValidVotes() / (double)province_results.get(j).getProvince().getConstituencySeats()) && \exists int m; 0 <= m && m < province_results.size(); \exists int n; 0 <= n && province_results.get(m).getPartyResult().size(); (province_results.get(m).getPartyResult().get(n).getParty().getPartyId() == national_results.get(i).getParty().getPartyId() && province_results.get(m).getPartyResult().get(n).getTotalVotes() >= (int)((double)province_results.get(m).getValidVotes() / (double)province_results.get(m).getProvince().getConstituencySeats()) && i != m;
	...
	public void determine_threshold() {
	...
	public void allocate_additional_seats_to_parties() {
		/*
	...
	//@ pre \exists int i; 0 <= i && i < national_results.size(); national_results.get(i).getParty().getPartyId() == p_pr.getParty().getPartyId();
	//@ pre p_pr != null;
	//@ pre p_pr.isDraw();
	//@ pre p_pr.getAdditionalSeats() > 0;
	//@ pre seats_total > ElectionConstants.SEATS_DENMARK;
	//@ post !p_pr.isDraw();
	//@ post \old(p_pr).getAdditionalSeats() = p_pr.getAdditionalSeats() + 1;
	//@ post \old(seats_total) = seats_total + 1;
	//@ post (seats_total == ElectionConstants.SEATS_DENMARK) ==> inner_status == ElectionStatus.STEP_3_RESOLVED;
	//@ assignable seats_total, national_results, inner_status;
	public void resolve_additional_seats(PartyResult p_pr) {
		...
		for (int i = 0; i < national_results.size(); i++) {
			...
			if (pres.getParty().getPartyId() == p_pr.getParty().getPartyId()) {
				if (pres.isDraw()) {
	...
	/**
	 * Step 3 (if there are too many seats taken):
	 * This method prunes down the number of seats taken down to the 
	 * max number of seats the parliament should have. 
	 * N.B. This should be a (pseudo) random algorithm, but since this in 
	 * the first place should not be a part of the application, then first 
	 * party in the list is never favoured
	 */
	//@ pre inner_status == ElectionStatus.STEP_3_DONE;
	//@ pre seats_total > ElectionConstants.SEATS_DENMARK;
	//@ post inner_status == ElectionStatus.STEP_3_RESOLVED;
	//@ post seats_total == ElectionConstants.SEATS_DENMARK;
	//@ assignable inner_status, seats_total, national_results;
	public void auto_resolve_additional_seats() {
		PartyResult pres;
	...
	//@ pre \forall int j; 0 <= j && j <= province_results.size(); province_results.get(j).getSeatsTaken() == 0;
	//@ pre \forall int i; 0 <= i && i <= province_results.size(); \forall int j; province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeats() == 0;
	//@ pre \forall int i; 0 <= i && i <= province_results.size(); \forall int j; province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getDivs().size() == province_results.get(i).getPartyResult().get(j).getConstituencySeats();
	//@ post additional_seats_taken <= ElectionConstants.ADDITIONAL_SEATS;
	//@ post isStep4resolved() ==> \forall int j; 0 <= j && j <= province_results.size(); province_results.get(j).getProvince().getAdditionalSeats() == province_results.get(j).getSeatsTaken();
	//@ post !isStep4resolved() ==> \exists int j; 0 <= j && j <= province_results.size(); province_results.get(j).getSeatsTaken() > province_results.get(j).getProvince().getAdditionalSeats();
	...
	public void allocate_additional_seats_to_provinces() {
	...
	//@ post \result.getPartyResults().size() > 0 && \result.getProvinceResults().size() > 0 && (\result.getPartyResults().size() > 1 || \result.getProvinceResults().size() > 1); 
	public StepFourDraw getStepFourDraw() {
		StepFourDraw draw;
		...
		for (int i = 0; i < province_results.size(); i++) {
			...
			for (int j = 0; j < pr.getPartyResult().size(); j++) {
				...
				tmp = pres.getDivs().get(pres.getDivs().size()-1).getRanking();
				if (tmp > 0 && tmp > ranking) {
					ranking = tmp;
				}
			}
		}
		...
		for (int i = 0; i < province_results.size(); i++) {
			...
			for (int j = 0; j < pr.getPartyResult().size(); j++) {
				...
				if (tmp == ranking) {
					 ...
					 * //TODO: document this properly in GUI section in report 
					 */
	...
	//@ pre \exists int i; 0 <= i && i < national_results.size(); national_results.get(i).getParty().getPartyId() == p_presn.getParty() && p_pr.getAdditionalSeatsTaken() > p_pr.getProvince().getAdditionalSeats() || p_presn.getAdditionalSeatsTaken() > p_presn.getAdditionalSeats();
	//@ post additional_seats_taken = \old(additional_seats_taken) - 1;
	//@ post \exists int i; 0 <= i && i < national_results.size(); national_results.get(i).getParty().getPartyId() == p_presn.getParty() && national_results.get(i).getAdditionalSeatsTaken() == \old(p_presn).getAdditionalSeatsTaken() - 1;
	//@ post \exists int i; 0 <= i && i < province_results.size(); province_results.get(i).getProvince().getProvinceId() == p_pr.getProvinceId() && province_results.get(i).getAdditionalSeatsTaken() == \old(p_pr).getAdditionalSeatsTaken() - 1;
	//@ post \forall int i; 0 <= i && i < province_results.size(); \forall int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); \forall int k; 0 <= k && k < province_results.get(i).getPartyResult().get(j).getDivs().size(); (province_results.get(i).getProvince().getProvinceId() != p_pr.getProvinceId() || province_results.get(i).getPartyResult().get(j).getParty().getPartyId() != p_presn.getParty().getPartyId()) ==> province_results.get(i).getPartyResult().get(j).getDivs().get(k).getRanking != rank;
	...
	public void resolveStep4draw(int rank, 
		 ...
		 * Check input
		 */
		if (!tooManyAdditionalSeatsTaken()) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
					"There must be a draw issue present to resolve");
		}
		...
		if (p_presn == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
					"The party result must be non null");
		}
		if (p_pr == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
					"The province result must be non null");
		}
		PartyResult presn;
		for (int i = 0; i < national_results.size(); i++) {
		...
		for (int i = 0; i < national_results.size(); i++) {
			...
			for (int j = 0; j < province_results.size(); j++) {
				...
				for (int k = 0; k < pr.getPartyResult().size(); k++) {
					...
					curr = presp.getDivs().get(presp.getDivs().size()-2);
	...
	public void auto_resolve_step_4_draw() {
		//TODO:
	}
	...
	//@ post (inner_status == ElectionStatus.STEP_5_DONE) ==> (\exists int i; 0 <= i && i < province_results.size(); \exists int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() > province_results.get(i).getPartyResult().get(j).getAdditionalSeats()); 
	//@ post (inner_status == ElectionStatus.STEP_5_RESOLVED) ==> (\forall int i; 0 <= i && i < province_results.size(); \forall int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() == province_results.get(i).getPartyResult().get(j).getAdditionalSeats()); 
	...
	//@ assignable inner_status, constituency_results;
	public void allocate_additional_seats_to_constituencies() {
		PartyResult pres;
		ProvinceResult pr;
		ConstituencyResult cr;
				...
				for (int j = 0; j < province_results.size(); j++) {
					...
					for (int p = 0; p < pr.getPartyResult().size(); p++) {
						...
						if (presp.getParty().getPartyId() == pres.getParty().getPartyId()) {
							 ...
							 */
							ranking = 1;
							ArrayList<Div> best = new ArrayList<Div>();
								...
								for (int k = 0; k < constituency_results.size(); k++) {
									...
									if (cr.getConstituency().getProvince().getProvinceId() 
										...
										for (int m = 0; m < cr.getPartyResult().size(); m++) {
											 ...
											 */
											if (presc.getParty().getPartyId() == pres.getParty().getPartyId()) {
	...
	//@ pre p_cr != null;
	//@ pre \exists int i; 0 <= i && i < p_cr.getPartyResult().size(); p_vp.getParty().getPartyId() == p_cr.getPartyResult().get(i).getParty().getPartyId();
	//@ pre \exists int i; 0 <= i && i < p_vp.getDivs().size(); p_vp.getDivs().get(i).getRanking() == ranking; 
	//@ pre \exists int i; 0 <= i && i < province_result.size(); province_result.get(i).getProvince().getProvinceId() == p_cr.getConstituency().getProvince().getProvinceId() && \exists int j; 0 <= j && j < province_result.get(i).getPartyResult().size(); province_result.get(i).getPartyResult().get(j).getParty().getPartyId() == p_pres.getParty().getPartyId() && province_result.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() > province_result.get(i).getPartyResult().get(j).getAdditionalSeats();
	...
	//@ assignable constituency_results, inner_status;
	//@ exceptional_behavior
	//@ pre inner_status != ElectionStatus.STEP_5_DONE || ranking <= 0 || p_vp == null || p_cr == null || (\forall int i; 0 <= i && i < p_cr.getPartyResult().size(); p_vp.getParty().getPartyId() != p_cr.getPartyResult().get(i).getParty().getPartyId()) || (\forall int i; 0 <= i && i < p_cr.getPartyResult().size(); \forall int j; 0 <= j && j < p_cr.getPartyResult().get(i).getDivs().size(); p_cr.getPartyResult().get(i).getParty().getPartyId() != p_vp.getParty().getPartyId() ==> p_cr.getPartyResult().get(i).getDivs().get(j).getRanking() != ranking) || \forall int i; 0 <= i && i < p_cr.getPartyResult().size(); p_vp.getParty().getPartyId() != p_cr.getPartyResult().get(i).getParty().getPartyId() ==> (\forall int j; 0 <= j && j < p_cr.getPartyResult().get(i).getDivs().size(); p_cr.getPartyResult().get(j).getDivs().get(j).getRanking() != ranking);
	//@ signals (DivsException d) (d instanceof DivsException);
	public void resolve_constituency_result(int ranking, 
				PartyResult p_pres, ConstituencyResult p_cr) throws DivsException {
		 ...
		 * a) arguments are null
		 * b) inner status is not the right one
		 * c) ranking is zero or less
		 * d) party does not exist in the party_result list of constituency result
		 * e) no div of others parties has the ranking.
		 * If all tests are ok then continue 
		 */
		if (p_pres == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
				"Argument 'p_vp' is null");
		}
		if (p_cr == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
				"Argument 'p_cr' is null");
		}
		if (inner_status != ElectionStatus.STEP_5_DONE) {
			throw new DivsException(DivsException.ILLEGAL_INPUT, 
					"Inner state is not 'step 4 done'");
		}
		if (ranking <= 0) {
			throw new DivsException(DivsException.ILLEGAL_INPUT, 
		...
		found = false;
		outer: for (int i = 0; i < p_cr.getPartyResult().size(); i++) {
			presc = p_cr.getPartyResult().get(i);
			if (presc.getParty().getPartyId() != p_pres.getParty().getPartyId()) {
				for (int j = 0; j < presc.getDivs().size(); j++) {
					curr = presc.getDivs().get(j);
	...
	/**
	 * Step 5 (if there are some ties in between two or more constituencies):
	 * This method prunes down the number of additional seats taken in a
	 * constituency so that the number of seats within each constituency is
	 * exactly the number of seats there should be.
	 * N.B. This should be a (pseudo) random algorithm, but since this in 
	 * the first place should not be a part of the application, then first 
	 * constituency in the list is never favoured
	 */
	//@ pre inner_status == ElectionStatus.STEP_5_DONE;
	//@ post inner_status == ElectionStatus.STEP_5_RESOLVED;
	//@ post \forall int i; 0 <= i && i < province_results.size(); \forall int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() == province_results.get(i).getPartyResult().get(j).getAdditionalSeats(); 
	//@ assignable inner_status, constituency_results;
	public void auto_resolve_constituency_result() {
		PartyResult pres;
		ProvinceResult pr;
		ConstituencyResult cr;
		PartyResult presc, presp;
		Div curr;
				...
				for (int j = 0; j < province_results.size(); j++) {
					...
					for (int p = 0; p < pr.getPartyResult().size(); p++) {
						...
						if (presp.getParty().getPartyId() == pres.getParty().getPartyId()) {
							 ...
							 * won in the province 
							 */
							additional_seats = presp.getAdditionalSeats();
							ranks = new int[additional_seats];
							
							/*
							 * While the counted number of additional seats is smaller
							 * than the actual number of additional seats taken in
							 * the province for the party...
							 */
							while (additional_seats < countAdditionalSeatsTakenInProvince(pr, pres)) {

								/*
								 * Find out how many time each ranking is used
								 */
								for (int k = 0; k < constituency_results.size(); k++) {
									cr = constituency_results.get(k);
									if (cr.getConstituency().getProvince().getProvinceId() 
										...
										for (int m = 0; m < cr.getPartyResult().size(); m++) {
											...
											if (presc.getParty().getPartyId() == pres.getParty().getPartyId()) {
												...
												for (int d = 0; d < presc.getDivs().size(); d++) {
													...
													/*
													 * If there is a ranking then it is interesting!
													 */
													if (curr.getRanking() > 0) {
														 ...
														 */
														if (ranks[curr.getRanking()-1] > 0) {

															/*
															 * Ranking exists and we push the
															 * current down by 1...
															 */
															curr.setRanking(curr.getRanking()+1);
															if (curr.getRanking() > additional_seats) {
															...
															/*
															 * Ranking does not exist and we
															 * cross it off the list 
															 */
															ranks[curr.getRanking()-1] += 1;													
														}
													}
												}
											}
		...
		/*
		 * Determine status and prepare next step
		 */
		inner_status = ElectionStatus.STEP_5_RESOLVED;
		this.prepare_step_six();
	}
	...
	//@ pre \forall int i; 0 <= i && i < constituency_results.size(); \forall int j; 0 <= j && j < constituency_results.get(i).getPartyResult().size(); constituency_results.get(i).getPartyResult().get(j).getTotalSeats() <= constituency_results.get(i).getPartyResult().get(j).getCandidateResult().size();
	...
	//@ pre \exists int i; 0 <= i && i < p_cr.getPartyResult().size(); p_cr.getPartyResult().get(i).getParty().getPartyId() == p_pr.getParty().getPartyId();
	//@ pre \exists int i; 0 <= i && i < p_pr.getCandidateResults().size(); p_pr.getCandidateResults().get(i).getCandidate().getCandidateId() == p_cres.getCandidate().getCandidateId();
	//@ post !p_cres.isElected();
	//@ post isStep6Resolved() ==> inner_status == ElectionStatus.STEP_6_RESOLVED;
	//@ assignable inner_status, constituency_results;
	//@ exceptional_behavior
	//@ pre inner_status != ElectionStatus.STEP_6_DONE || p_cr == null || p_pr != null || p_cres != null || !p_cres.isElected() || p_pres.getNoOfElected() <= p_pres.getTotalSeats() || \forall int i; 0 <= i && i < p_cr.getPartyResult().size(); p_cr.getPartyResult().get(i).getParty().getPartyId() != p_pr.getParty().getPartyId() || \forall int i; 0 <= i && i < p_pr.getCandidateResults().size(); p_pr.getCandidateResults().get(i).getCandidate().getCandidateId() != p_cres.getCandidate().getCandidateId();
	//@ signals (DivsException d) (d instanceof DivsException);
	public void resolve_candidates(ConstituencyResult p_cr, 
							PartyResult p_pres, CandidateResult p_cres)
		 ...
		 * If all tests are ok then continue 
		 */
		if (inner_status != ElectionStatus.STEP_6_DONE) {
				throw new DivsException(DivsException.ILLEGAL_INPUT, 
						"Inner state is not 'step 6 done'");
		}
		if (p_cr == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
				"Argument 'p_cr' is null");
		...
		if (p_pres == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
				"Argument 'p_pres' is null");
		}
		if (p_cres == null) {
			throw new DivsException(DivsException.ILLEGAL_INPUT,
				"Argument 'p_cres' is null");
		}
		...
		outer: for (int i = 0; i < constituency_results.size(); i++) {
			...
			for (int j = 0; j < cr.getPartyResult().size(); j++) {
				...
				if (pres.getParty().getPartyId() == p_pres.getParty().getPartyId()) {
					...
					for (int k = 0; k < pres.getCandidateResults().size(); k++) {
						cres = pres.getCandidateResults().get(k);
						if (cres.getCandidate().getCandidateId() == p_cres.getCandidate().getCandidateId()) {
	...
	/**
	 * Step 6 (if there are too many candidates elected):
	 * After candidates are selected there might be too many of them. This
	 * method prunes down the list to exact number of seats each party has
	 * got in each constituency
	 * N.B. This should be a (pseudo) random algorithm, but since this in 
	 * the first place should not be a part of the application, then first 
	 * party in the list is never favoured
	 */
	//@ pre inner_status == ElectionStatus.STEP_6_DONE;
	//@ post inner_status == ElectionStatus.STEP_6_RESOLVED;
	//@ pre \forall int i; 0 <= i && i < constituency_results.size(); (\exists int j; 0 <= j && j < constituency_results.get(i).getPartyResult().size(); constituency_results.get(i).getPartyResult().get(j).getNoOfElected() > constituency_results.get(i).getPartyResult().get(j).getTotalSeats());
	//@ post \forall int i; 0 <= i && i < constituency_results.size(); (\forall int j; 0 <= j && j < constituency_results.get(i).getPartyResult().size(); constituency_results.get(i).getPartyResult().get(j).getNoOfElected() == constituency_results.get(i).getPartyResult().get(j).getTotalSeats());
	//@ assignable inner_status, constituency_results;
	public void auto_resolve_candidates() {
		ConstituencyResult cr;
		PartyResult pres;
		CandidateResult cres;
		for (int i = 0; i < constituency_results.size(); i++) {
	...
	private void prepare_step_three () {
	...
	private void prepare_step_four() {
		ProvinceResult pr;
		ConstituencyResult cr;
		PartyResult presp, presc;
	...
	private void prepare_step_five() {
		ProvinceResult pr;
		ConstituencyResult cr;
		PartyResult presp, presc;
	...
	//@ post \exists int i; 0 <= i && i < constituency_results.size(); constituency_results.get(i).getProvince().getProvinceId() == pr.getProvince().getProvinceId() ==> \result >= constituency_results.get(i).getAdditionalSeats(); 
	private int countAdditionalSeatsTakenInProvince(
			/*@ pure @*/ ProvinceResult pr, /*@ pure @*/ PartyResult p_pres) {
		ConstituencyResult cr;
		PartyResult pres;
		int count = 0;
		for (int k = 0; k < constituency_results.size(); k++) {
			...
			if (cr.getConstituency().getProvince().getProvinceId() 
							== pr.getProvince().getProvinceId()) {
				for (int m = 0; m < cr.getPartyResult().size(); m++) {
					pres = cr.getPartyResult().get(m);
					if (pres.getParty().getPartyId() == p_pres.getParty().getPartyId()) {
						count += pres.getAdditionalSeats();
					}
				}
			}
		}
	 ...
	 */
	//@ post \exists int i; 0 <= i && i < national_results.size(); national_results.isThresholdPassed() ==> \result > 0; 
	//@ post \forall int i; 0 <= i && i < national_results.size(); national_results.isThresholdPassed() ==> \result == national_results.size(); 
	//@ post \forall int i; 0 <= i && i < national_results.size(); !national_results.isThresholdPassed() ==> \result == 0; 
	...
	//@ post \exists int i; 0 <= i && i < national_results.size(); national_results.get(i).getAdditionalSeatsTaken() > national_results.getAdditionalSeats() ==> \result == true;
	 ...
	 */
	//@ pre national_results.size() > 0;
	//@ post (\exists int i; 0 <= i && i < national_results.size(); vp.getParty().getPartyId() == national_results.get(i).getParty().getPartyId()) ==> \result != null;
	//@ post (\forall int i; 0 <= i && i < national_results.size(); vp.getParty().getPartyId() != national_results.get(i).getParty().getPartyId()) ==> \result == null;
	...
	//@ assignable province_results, additional_seats_taken, seat_no;
	private void findAdditionalSeats() {
		 ...
		 */
		int ranking = additional_seats_taken;
		int count = 0;
		for (int i = 0; i < province_results.size(); i++) {
			pr = province_results.get(i);
			for (int j = 0; j < pr.getPartyResult().size(); j++) {
	...
	private List<PartyResult> partiesNotRunningInAllProvinces() {
		...
		ProvinceResult pr;
		PartyResult pres, presp;
		boolean found;
		for (int i = 0; i < national_results.size(); i++) {
			pres = national_results.get(i);
			if (pres.isThresholdPassed() && pres.getAdditionalSeats() > 0) {
				found = false;
				for (int j = 0; j < province_results.size(); j++) {
					pr = province_results.get(j);
					for (int k = 0; k < pr.getPartyResult().size(); k++) {
						presp = pr.getPartyResult().get(k);
						if (pres.getParty().getPartyId() == presp.getParty().getPartyId()) {
							found = true;
						}
					}
				}
	 ...
	 */
	//@ post (\exists int i; 0 <= i && i < province_results.size(); province_results.get(i).getAdditionalSeatsTaken() != province_results.get(i).getProvince().getAdditionalSeats() || \exists int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() != province_results.get(i).getPartyResult().get(j).getAdditionalSeats()) ==> \result == false; 
	//@ post (\forall int i; 0 <= i && i < province_results.size(); province_results.get(i).getAdditionalSeatsTaken() == province_results.get(i).getProvince().getAdditionalSeats() && \forall int j; 0 <= j && j < province_results.get(i).getPartyResult().size(); province_results.get(i).getPartyResult().get(j).getAdditionalSeatsTaken() == province_results.get(i).getPartyResult().get(j).getAdditionalSeats()) ==> \result == true; 
	...
	private /*@ pure @*/ boolean isStep6Resolved() {
		...
		for (int i = 0; i < constituency_results.size(); i++) {
			...
			for (int j = 0; j < cr.getPartyResult().size(); j++) {
				pres = cr.getPartyResult().get(j);
				if (!pres.isResolved()) {
					//System.out.println("hmm :-/ "+cr.getConstituency().getName()+"\t"+pr.getNoOfElected()+"!="+pr.getTotalSeats());
					return false;
				}
			}
		}
	...
	/**
	 * After step one it is determined between which parties lots must 
	 * be drawn. This method informs per constituency which parties
	 * are involved
	 * @param cr Constituency result
	 * @return Returns the number of parties involved in the draw
	 */
	//@ post (\exists int i; 0 <= i && i < cr.getPartyResult().size(); cr.getPartyResult().get(i).isDraw() && (\exists int j; 0 <= j && j < cr.getPartyResult().size(); cr.getPartyResult().get(j).isDraw() && i != j)) ==> \result.size() > 1;
	private /*@ pure @*/ List<PartyResult> getPartyDraw(
				/*@ non_null @*/ ConstituencyResult cr) {
		List<PartyResult> retval = new ArrayList<PartyResult>();
		for (int i = 0; i < cr.getPartyResult().size(); i++) {
			if (cr.getPartyResult().get(i).isDraw()) {
				retval.add(cr.getPartyResult().get(i));
			}
		}
		return retval;
	}
	...
	//@ pre province_results.size() == ElectionConstants.NO_OF_PROVINCES;
	//@ pre national_results.size() > 0;
	//@ post pr.getConstituencySeats() > 0 ==> \result == true;
	//@ post (\exists int i; 0 <= i && i < province_results.size(); \exists int p; 0 <= p && p < province_results.get(i).getPartyResult().size(); (province_results.get(i).getPartyResult().get(p).getParty().getPartyId() == pr.getParty().getPartyId()) && (((double) province_results.get(i).getPartyResult().get(p).getValidVotes() / (double) province_results.get(i).getProvince().getConstituencySeats()) <= (double) province_results.get(i).getTotalVotes()) && (\exists int ii; 0 <= ii && ii < province_results.size(); \exists int pp; 0 <= pp && pp < province_results.get(ii).getPartyResult().size(); (((double) province_results.get(ii).getPartyResult().get(pp).getValidVotes() / (double) province_results.get(ii).getProvince().getConstituencySeats()) <= (double) province_results.get(ii).getTotalVotes()) && (p == pp && i != ii))) ==> \result == true;
	//@ post (( (double) pr.getTotalVotes() / (double) total_valid_votes * 100.0) >= 2.0) ==> \result == true;
	...
	//@ post (\exists int i; 0 <= i && i < national_results.size(); national_results.get(i).isThresholdPassed() && pres.getAdditionalSeatsTaken() < pres.getAdditionalSeats()) ==> \result == false;  
	//@ post (\forall int i; 0 <= i && i < national_results.size(); national_results.get(i).isThresholdPassed() && pres.getAdditionalSeatsTaken() >= pres.getAdditionalSeats()) ==> \result == true;  
